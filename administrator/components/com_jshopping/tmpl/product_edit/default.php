...
<?php
	$pane = null;
	if ($this->product->parent_id==0 || isset($this->ptab_descr)){
        include(dirname(__FILE__)."/description.php");
     }
     if ($this->product->parent_id==0 || isset($this->ptab_info)){
	...
	if ($jshopConfig->admin_show_attributes && ($this->product->parent_id==0 || isset($this->ptab_attr))){
		include(dirname(__FILE__)."/attribute.php");
	}
	if ($jshopConfig->admin_show_freeattributes && ($this->product->parent_id==0 || isset($this->ptab_free_attr))){
		include(dirname(__FILE__)."/freeattribute.php");
	}
	include(dirname(__FILE__)."/images.php");
	if ($jshopConfig->admin_show_product_video && ($this->product->parent_id==0 || isset($this->ptab_video))){
		include(dirname(__FILE__)."/videos.php");
	}
	if ($jshopConfig->admin_show_product_related && ($this->product->parent_id==0 || isset($this->ptab_related))){
		include(dirname(__FILE__)."/related.php");
	}
	if ($jshopConfig->admin_show_product_files) {
		include(dirname(__FILE__)."/files.php");
	}
	if ($jshopConfig->admin_show_product_extra_field && ($this->product->parent_id==0 || isset($this->ptab_extra_field))){
